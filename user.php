...
function action_default ()
{
	...
	$ecs = $GLOBALS['ecs'];
	$user_id = $_SESSION['user_id'];
	/* 代码注释 by www.cfweb2015.com _star 路由前已调用会员信息，去掉重复调用*/
	$rank = get_rank_info();
	/* 	include_once (ROOT_PATH . 'includes/lib_clips.php');
	 if($rank = get_rank_info())
	 {
	...
	/* 代码注释 by www.cfweb2015.com _end */
	...
	/* 代码增加--cb--推荐分成-- by www.cfweb2015.com _star */
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_name= '$rn'");
    ...
    /* 代码修改 By  www.cfweb2015.com Start */
...
function action_order_detail ()
{
    ...
    /* 代码删除 By  www.cfweb2015.com Start */
//	/* 未发货，未付款时允许更换支付方式 */
...
//	$order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
	...
	foreach($goods_list as $goods_key => $goods_val)
	{
		...
		/* 代码增加_start By www.cfweb2015.com _20150804 */
		...
		/* 代码增加_end By www.cfweb2015.com _20150804 */
...
/* 商品评价/晒单 增加 by www.cfweb2015.com */
...
/* 余额额支付密码_添加_START_www.cfweb2015.com */
...
function action_fenxiao(){
   // expend_order_log(182, 65);
	...
	$tixianlist5 = get_tixian_list($user_id, 5);
	//var_dump($fenxiao);
	//{{{获取推广分销码
    if($baseinfo && $baseinfo['expend_code']){
       $expend_code = $baseinfo['expend_code'];
    }else{
        //注册的时候已插入user_expend表中
        //第一次达到目标，生产推广码
        $expend_code = get_user_expend_code($user_id);
        //var_dump($expend_code);
        set_user_base_expend_code($user_id, 'expend_code', $expend_code);
        $baseinfo['expend_code'] = $expend_code;
    } 
    //}}} end 推广分销码
    
    //{{{ 获得佣金记录 
    if($exec == 'yongjin_log'){
		$yongjinlist = get_yongjin_list($user_id, 1000);
		$smarty->assign("yongjinlist",$yongjinlist);
	}
    //}}} end 佣金记录
    
    //{{{ 获得推广记录
    if($exec == 'show_expend_users'){
		$sql = "SELECT id, user_id, user_name, expend_ctime,FROM_UNIXTIME(expend_ctime,'%Y年%m月%d') as expend_ctime_format, is_valid_user from ".$ecs->table('user_expend')." where expend_pid = $user_id and is_valid_user = 1 ";
		//echo $sql;
		$expend_users_list = $db->getAll($sql);
		
		$smarty->assign('expend_users_list',$expend_users_list);
	}
    //}}} end 推广记录
    
    //{{{ 获得分红记录
    if($exec == "bonus_list"){
		$bonuslist = get_bonus_list($user_id, 1000);
		$smarty->assign("bonuslist",$bonuslist);
	}
    //}}} end 分红记录
    
    //{{{ 体现记录
    if($exec == 'tixian'){
		$tixian_money = floatval(isset($_POST['tixian_money'])?$_POST['tixian_money']:0);
		$tixian_type = isset($_POST['tixian_type'])?$_POST['tixian_type']:0;
		$tixian_account = isset($_POST['tixian_account'])?$_POST['tixian_account']:0;
		if($tixian_money >0 && $tixian_type && $tixian_account){
			if($tixian_money >0 && $tixian_money>=floatval($fenxiao['tixian_limit']) && $tixian_money<=floatval($baseinfo['expend_amount'])){
				
				//提现申请
				$shouxufei = floatval($tixian_money*($fenxiao['tixian_shouxufei']/100));
				$shijidaozhang = $tixian_money - $shouxufei;
				if(put_tixian($user_id, $tixian_money,$tixian_type,$tixian_account,$shijidaozhang,$shouxufei)){
					show_message("提交成功，我们将尽快审核");
				}else{
					show_message("您当前已有提现申请，请完成后再提交");
				}
			}else{
				
				show_message("提现金额不能超过".$baseinfo['expend_amount']);
			}
		}
	}
	if($exec == "tixian_list"){
		$tixianlist = get_tixian_list($user_id, 1000);
		$smarty->assign("tixianlist",$tixianlist);
	}
	...
	$fenxiao_guize = formatFenxiao($fenxiao);
	//var_dump($baseinfo);
	$smarty->assign("fenxiao", $fenxiao);
	$smarty->assign("fenxiao_guize", $fenxiao_guize);
	$smarty->assign("baseinfo", $baseinfo);
	$smarty->assign("expend_amount_rest", price_format($expend_amount_rest));
	$smarty->assign("tixianlist5", $tixianlist5);
...
function formatFenxiao($fengxiao){
	$arr = array();
	if($fengxiao && $fengxiao['dataitem']){
		foreach($fengxiao['dataitem'] as $k=>$v){
			$id = str_replace("zhitui_leve","",$k);
			$keyar[] = $v['sbs'.$id."_share_people_num"];
			//echo $v['shs'.$id."_share_people_num"];
			$bonus_str = "";
			if($v['sbs'.$id."_share_people_num"] >= 56){
				$bonus_str = " (享受分红 5%)";
			}else if($v['sbs'.$id."_share_people_num"] >= 32){
				$bonus_str = " (享受分红 2%)";
			}else if($v['sbs'.$id."_share_people_num"] >= 18){
				$bonus_str = " (享受分红 1%)";
			}
			$arr[$v['sbs'.$id."_share_people_num"]] = array(
				"id"=>$id,
				"people_num"=>$v['sbs'.$id."_share_people_num"],
				"l1"=>$v['sbs'.$id."_leve1"],
				"l2"=>$v['sbs'.$id."_leve2"],
				"l3"=>$v['sbs'.$id."_leve3"],
				"l1_bonus"=>$bonus_str
	...
	array_multisort($keyar, SORT_ASC, $arr);
...
//}}} end 分销
/* 账户安全_end_www.cfweb2015.com */
/* 余额额支付密码_添加_END_www.cfweb2015.com */

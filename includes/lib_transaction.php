...
function get_profile($user_id)
{
    ...
    $info['huiyuanka']	      = $infos['huiyuanka'];
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, shipping_time," .
			" supplier_id, rebate_id, parent_order_id,	  ". //代码增加  By  www.cfweb2015.com
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
			...
			'is_suborder' => $row['parent_order_id'] ? "(子订单)" : "",  //代码增加   By  www.cfweb2015.com
                       'handler'        => $row['handler']);		
...
function affirm_received($order_id, $user_id = 0)
{
    ...
    else
    {
        ...
        if ($GLOBALS['db']->query($sql))
        {
			...
			/* 设置父用户成功有效用户 */
			//set_user_to_fenxiao($user_id);
			/* 计算此用户消费总金额，看是否达到分红级别 */
			//check_user_fenhong_status($user_id,$order_id);
			/* 根据推广的人数，更新推广人数信息，同时看是否达到分红级别*/
			//set_user_bonus_level($user_id);
			/* 购物后， 则成为分佣人员 */
            /* 收货，则创建分佣记录 */

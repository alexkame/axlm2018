...
$is_show_sanji_fenxiao = 1;
$is_show_fanxian = 1;
$is_show_fenhong = 0;
$smarty->assign("is_show_sanji_fenxiao", $is_show_sanji_fenxiao);
$smarty->assign("is_show_fanxian", $is_show_fanxian);
$smarty->assign("is_show_fenhong", $is_show_fenhong);
...
$ui_arr[] = "account_yongjin";
$ui_arr[] = "account_fenhong";
$ui_arr[] = "account_expendcode";
$ui_arr[] = "account_expendlist";
...
function action_profile()
{
	...
	$smarty->assign('huiyuanka', $huiyuanka);
	$smarty->display('user_transaction.dwt');
}
...
function action_act_edit_profile()
{
	...
	$huiyuanka = trim($_POST['huiyuanka']);
...
// 	);
	...
	{
		if($huiyuanka){
			$sql = 'UPDATE ' . $ecs->table('users') . " SET `user_name`='$user_name', `huiyuanka`='$huiyuanka' WHERE `user_id`='" . $_SESSION['user_id'] . "'";
		}else{
...
function action_account_expendcode(){
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
...
function action_account_expendlist()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$baseinfo = get_user_base_expend_code($user_id);
	$sql = "select count(id) from ".$ecs->table('user_expend')." where expend_pid = $user_id";
	$expend_user_total = $db->getOne($sql);
	$baseinfo['expend_user_total'] = $expend_user_total;
	...
	$list = array();
	$sql = "SELECT ue.id,ue.user_id, ue.expend_ctime,u.user_name,ue.is_valid_user FROM " . $ecs->table('user_expend') . " ue ".
			"left join ".$ecs->table('users')." u on u.user_id = ue.user_id ".
			" WHERE expend_pid = $user_id order by ue.id desc ";
	//echo $sql;
	$res = $GLOBALS['db']->selectLimit($sql, $pager['size'], $pager['start']);
	while($row = $db->fetchRow($res))
	{
	
		$row['expend_ctime'] = date("Y-m-d H:i",$row['expend_ctime']);
		if($row['is_valid_user'] == 1){
			$row['is_valid_user'] = "有效用户";
		}else{
			$row['is_valid_user'] = "";
		}
		$list[] = $row;
...
/* 会员佣金界面 */
function action_account_yongjin()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
	$sql = "select count(id) from ".$ecs->table('user_expend_log')." where expend_user_id = $user_id";
	$record_count = $db->getOne($sql);
	$pager = get_pager('user.php', array(
		'act' => $action
	), $record_count, $page);
	...
	$list = array();
	$sql = "SELECT ue.id,ue.order_id, o.order_sn, ue.expend_user_id,ue.order_amount, ue.expend_amount, ue.status,ue.ctime FROM " . $ecs->table('user_expend_log') . " ue ".
			"left join ".$ecs->table('order_info')." o on o.order_id = ue.order_id ".
			" WHERE expend_user_id = $user_id order by ue.id desc ";
	//echo $sql;
	$res = $GLOBALS['db']->selectLimit($sql, $pager['size'], $pager['start']);
	while($row = $db->fetchRow($res))
	{
...
function action_account_fenhong()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
	$sql = "select count(ub.id) from ".$ecs->table('user_bonus_log')." ub ".
			"left join ".$ecs->table('bonus_total')." b on b.id = ub.bonus_id ".
			" WHERE ub.user_id = $user_id and ub.status = 1";
	$record_count = $db->getOne($sql);
	$pager = get_pager('user.php', array(
		'act' => $action
	), $record_count, $page);
	...
	$list = array();
	$sql = "SELECT ub.id,ub.user_id,ub.user_bonus_level,ub.bonus_money,ub.ctime,ub.bonus_id,b.stime, b.etime  FROM " . $ecs->table('user_bonus_log') . " ub ".
			"left join ".$ecs->table('bonus_total')." b on b.id = ub.bonus_id ".
			" WHERE ub.user_id = $user_id and ub.status = 1 order by ub.id desc ";
	//echo $sql;
	$res = $GLOBALS['db']->selectLimit($sql, $pager['size'], $pager['start']);
	while($row = $db->fetchRow($res))
	{
		...
		$row['user_bonus_level'] = $row['user_bonus_level']."%";
		$row['bonus_money_format'] = price_format($row['bonus_money']);
